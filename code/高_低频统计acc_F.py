...
for label in group1_labels:
    if label in VERBALIZER_INDEX_LABEL:
        label_to_index[label] = VERBALIZER_INDEX_LABEL[label]

# 修改后的数据集类，使用label_to_index映射标签
class TestDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length, label_to_index):
        self.text_list = []
        self.label_list = []
        ...
        with open(data_path) as f:
            ...
            for idx, row in enumerate(reader):
                label, text = row
                if label in label_to_index:  # 只使用label_to_index中存在的标签
                    self.text_list.append(text)
                    self.label_list.append(label_to_index[label])
...
# 评估函数，确保只计算label_to_index中的标签
criterion = nn.CrossEntropyLoss()


# 评估函数，确保只计算label_to_index中的标签
def evaluate_model(model, test_loader, criterion, label_to_index):
    model.eval()
    all_preds = []
    all_labels = []
    total_loss = 0.0
    ...
    # 过滤出只包含label_to_index中的标签
    filtered_labels = []
    filtered_preds = []
    for label, pred in zip(all_labels, all_preds):
        if label in label_to_index.values():  # 只保留group1标签
            filtered_labels.append(label)
            filtered_preds.append(pred)

    # 检查过滤后的标签和预测值的数量是否一致
    print(f"All Labels: {len(all_labels)}, All Predictions: {len(all_preds)}")
    print(f"Filtered Labels: {len(filtered_labels)}, Filtered Predictions: {len(filtered_preds)}")
    if len(filtered_labels) != len(filtered_preds):
        raise ValueError(f"Length mismatch: {len(filtered_labels)} != {len(filtered_preds)}")

    # 计算评估指标
    accuracy = accuracy_score(filtered_labels, filtered_preds)
    precision, recall, f1, _ = precision_recall_fscore_support(filtered_labels, filtered_preds, average='weighted')
    macro_precision, macro_recall, _, _ = precision_recall_fscore_support(filtered_labels, filtered_preds, average='macro')
    micro_precision, micro_recall, _, _ = precision_recall_fscore_support(filtered_labels, filtered_preds, average='micro')

    # 生成报告，忽略未出现的标签
    labels = list(set(filtered_labels))  # 测试集中实际出现的标签
    target_names = [key for key in label_to_index.keys() if label_to_index[key] in labels]
    report = classification_report(filtered_labels, filtered_preds, labels=labels, target_names=target_names, zero_division=0)

    return total_loss / len(test_loader), accuracy, precision, recall, f1, macro_precision, macro_recall, micro_precision, micro_recall, report
...
def parse_arguments():
    ...
    parser.add_argument('--model_path', type=str, required=False, default='/home/icisee/CXL/pythonprojects/CrimePrediction/BeterModel/BigModel/betterModel/model10.pth', help='Path to the saved model')
    ...
    parser.add_argument('--test_data_path', type=str, required=False, default='/home/icisee/CXL/pythonprojects/CrimePrediction/Data_Clearning/big_194_test0.1.csv',help='Path to the test data')
...
if __name__ == '__main__':
    ...
    # 加载模型和分词器
    model = torch.load(args.model_path, map_location='cuda:0')  # 确保模型在cuda:0上
    # 如果只使用单卡评估
    model = model.module  # 提取原始模型
    model = model.to('cuda:0')  # 确保在cuda:0上
